    ...
    <body>
        <div class="main-wrapper">
            <div class="app" id="app">
                ...
                <article class="content board-compTable-page">
                    <section class="section">
                        ...
                        <nav>
                            ...
                            <a class="nav-item nav-link active" id="nav-icm-tab" data-toggle="tab" href="#nav-icm" role="tab" aria-controls="nav-icm" aria-selected="true">หลักสูตรการจัดการสารสนเทสและคอมพิวเตอร์</a>
                            <a class="nav-item nav-link" id="nav-ecm-tab" data-toggle="tab" href="#nav-ecm" role="tab" aria-controls="nav-ecm" aria-selected="false">หลักสูตรการจัดการพาณิชอีเล็กทรอนิกส์</a>
                            </div>
                        </nav>
                        <div class="tab-content" id="nav-tabContent">

                        <!-- icm tab -->
                        <div class="tab-pane fade show active" id="nav-icm" role="tabpanel" aria-labelledby="nav-icm-tab">
                                <!-- Example DataTables Card-->
                            <div class="card mb-3">
                            <div class="card-header">
                                <i class="fa fa-table"></i> &emsp; สถานประกอบการที่เปิดประจำภาคศึกษา 
                                <button class="btn btn-primary ml-auto addToBooking" name="add" majorid="1">เพิ่มสถานประกอบการให้นักศึกษาจอง</button>
                                <!-- <form>
                                <div class="form-group row">
                                    <label for="inputPassword" class="col-sm-2 col-form-label">ภาคการศึกษา</label>
                                    <div class="col-sm-4">
                                    <select class="form-control" id="stdterm">
                            ...
                            <div class="card-body">
                                <!-- <button class="btn btn-primary"name="add"id="addToBooking"data-toggle="modal"data-target="#modalListToBooking">เพิ่มสถานประกอบการให้นักศึกษาจอง</button>
                                <br><br> -->
                                <div class="table-responsive">

                                <?php
                                    require_once('conn_mysql.php');
                                    $sql = "SELECT*FROM company,booking where company.companyID=booking.companyID AND booking.majorID =1 AND booking.semesterID= $semesterID";
                                    $result=mysqli_query($conn,$sql)or die("Query failed");
                                    
                                    if(mysqli_num_rows($result)>0){
                                        print'<table class="table table-bordered table-sm table-hover" id="dataTable-icm" width="100%" cellspacing="0">';
                                        echo'<thead>
                                            <tr>
                                        ...
                                        while($row=mysqli_fetch_assoc($result)){
                                            ...
                                            // echo"<td>",$row["companyAddress"]," ต. ",$row["companyTambol"]," อ. ",$row["companyAmpo"]," จ. ",$row["companyChangwat"]," รหัสไปรษย์ณี ",$row["companyZipCode"],"</td>\n";
                                            echo"<td>",$row["companyChangwat"],"</td>\n";
                                            echo'<td class="text-center">'.$row["setQuota"].'</td>';
                                            $companyID = $row["companyID"];
                                            echo'<td class="text-center"> <button class="btn btn-primary btn-sm editData"data-major="ICM" quota="'.$row["setQuota"].'" id="',$row["companyID"],'">แก้ไข</button>';
                                            echo'   <button class="btn btn-danger btn-sm deleteDataBooking" data-name ="'.$row["companyName"].'" id="',$row["companyID"],'" major-id="1">delete</button>';
                                            echo'   <button class="btn btn-warning btn-sm details-control" id="',$row["companyID"],'" booking-id="'.$row["openBookingID"].'" clicker="board">ดูผู้จอง</button></td>';
                                            // echo'   <button class="btn btn-warning btn-sm viewStdbookInfo" id="',$row["companyID"],'">ดูผู้จอง</button></td>'; //สำหรับ allert Modal
                                            print"\t</tr>\n";
                            ...
                            <!-- ecm tab -->
                        <div class="tab-pane fade" id="nav-ecm" role="tabpanel" aria-labelledby="nav-ecm-tab">
                                <!-- Example DataTables Card-->
                            <div class="card mb-3">
                                <div class="card-header">
                                    <i class="fa fa-table"></i>&emsp; สถานประกอบการที่เปิดประจำภาคศึกษา 
                                    <button class="btn btn-primary ml-auto addToBooking"name="add" majorid="2">เพิ่มสถานประกอบการให้นักศึกษาจอง</button>
                                    <!-- <form>
                                    <div class="form-group row">
                                        <label for="inputPassword" class="col-sm-2 col-form-label">ภาคการศึกษา</label>
                                        <div class="col-sm-4">
                                ...
                                <div class="card-body">
                                    ...
                                    <?php
                                        ...
                                        $sql="SELECT*FROM company,booking where company.companyID=booking.companyID AND booking.majorID =2 AND booking.semesterID= $semesterID";
                                        $result=mysqli_query($conn,$sql)or die("Query failed");
                                        
                                        if(mysqli_num_rows($result)>0){
                                        print'<table class="table table-bordered table-sm" id="dataTable-ecm" width="100%" cellspacing="0">';
                                        echo'<thead>
                                        <tr>
                                        <th>ชื่อสถานประกอบการ</th>
                                        ...
                                        echo'<td class="text-center">'.$row["setQuota"].'</td>';
                                        $companyID = $row["companyID"];
                                        echo'<td class="text-center"> <button class="btn btn-primary btn-sm editData"data-major="ECM" quota="'.$row["setQuota"].'" id="',$row["companyID"],'">แก้ไข</button>';
                                        echo'   <button class="btn btn-danger btn-sm deleteDataBooking" data-name ="'.$row["companyName"].'" id="',$row["companyID"],'"major-id="2">delete</button>';
                                        echo'   <button class="btn btn-warning btn-sm details-control" id="',$row["companyID"],'" booking-id="'.$row["openBookingID"].'" clicker="board">ดูผู้จอง</button></td>';
                                        // echo'   <button class="btn btn-warning btn-sm viewStdbookInfo" id="',$row["companyID"],'">ดูผู้จอง</button></td>'; //สำหรับ allert Modal
                                        print"\t</tr>\n";
        ...
        <script src="js/comptostd-book.js"></script>
        <script>
        // wire up shown event
