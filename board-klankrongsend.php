    ...
    <body>
        <div class="main-wrapper">
            <div class="app" id="app">
                ...
                <article class="content board-klankrong-waiting-page">
                    <section class="section">
                        ...
                        <div class="breadcrumb">
                            <li class="breadcrumb-item">
                                <a href="board-klankrong-waiting.php">กลั่นกรองสถานประกอบการ</a>
                            </li>
                        ...
                        <div class="row">
                            <div class="col-lg-8">
                                ...
                                <div class="detail">
                                    ...
                                    $std=$row['stdID'];
                                    //ตรวจ กลั่นกรองที่ส่งมา เป็นครั้งที่เท่าไหร่ รวมถึง กรณีมี่หลายครั้ง แต่ละครั้งเป็นของครั้งที่เท่าไหร่
                                    // $xsql = "SELECT klankrongID FROM `stdklankrong` WHERE stdID=".$row['stdID']."ORDER BY klankrongID  DESC  LIMIT 1";
                                    $xsql = "SELECT klankrongID FROM `stdklankrong` WHERE stdID= $std ORDER BY klankrongID  DESC LIMIT 1";
                                    $xresult=mysqli_query($conn,$xsql)or die(mysqli_error($conn));
                                    if($xresult->num_rows > 0){
                                        $i=1;  
                                        $xrow = mysqli_fetch_array($xresult,MYSQLI_ASSOC);
                                        if ( $xrow['klankrongID'] ==$row['klankrongID']) {
                                            $explain = '';
                                            $btndisabled='';
                                        }else{
                                            $explain = '<br><b class="text-danger">ไม่สามารถดำเนินการได้ เนื่องจากไม่ใช่ข้อมูลการกลั่นกรองครั้งล่าสุดของนักศึกษาคนนี้</b>';
                                    ...
                                    <div class ="text-center">
                                        <button type="button" class="btn btn-success approve"<?php echo $btndisabled;?>>อนุมัติ</button>
                                        <button type="button" class="btn btn-danger not-approve"<?php echo $btndisabled;?>>ไม่อนุมัติ</button>
                                        <?php echo $explain;?>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <?php
                                        ...
                                        if($result2->num_rows >1){  
                                            ...
                                            while($row2 = mysqli_fetch_array($result2,MYSQLI_ASSOC)){
                                                ...
                                                echo '<td>';
                                                if ($row2['majorID']==1) {
                                                    echo ' <a href="board-klankrongsend.php?klankrongID='.$row2['klankrongID'].'">'.$row2['stdFirstname'].' '.$row2['stdLastname'].'</a></td>';
                                                    echo '<td>ICM</td>';
                                                    if ($row2['approve']=='yes') {
